...
public class GuardMeAction extends PlayerFunctionAction {
    ...
    private static boolean activateFollow(AbstractBuildingGuards guards, ServerPlayer player, UUID playerId) {
        try {
            ...
            String previous = me.lovkar.errands.GuardSettings.value(setting, GuardTaskSetting.PATROL);
            String previousMode = null;
            try {
                GuardFollowModeSetting fm = guards.getSetting(AbstractBuildingGuards.FOLLOW_MODE);
                if (fm != null) {
                    previousMode = me.lovkar.errands.GuardSettings.value(fm, null);
                    me.lovkar.errands.GuardSettings.set(fm, GuardFollowModeSetting.TIGHT); // stay close
                }
            } catch (Throwable ignored) {
            }
            if (!me.lovkar.errands.GuardSettings.set(setting, GuardTaskSetting.FOLLOW)) {
