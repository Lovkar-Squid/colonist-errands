        ...
        try {
            ...
            if (inProgress.isEmpty()) {
                ...
                for (ILocalResearch r : inProgress) {
                    ...
                    // Lovkar: the NAME rarely says what a research does - "Improved
                    // Swords" is what unlocks the Combat Academy. Say the effect.
                    String does = "";
                    try {
                        java.util.List<String> parts = new java.util.ArrayList<>();
                        for (var effect : IGlobalResearchTree.getInstance()
                                .getResearch(r.getBranch(), r.getId()).getEffects()) {
                        ...
                        does = String.join("; ", parts);
                    } catch (Throwable ignored) {
                    }
                    if (!first) sb.append("; ");
                    sb.append("'").append(name).append("'");
                    if (!does.isEmpty()) {
                        sb.append(" (which ").append(does.toLowerCase()).append(")");
                    }
                    sb.append(" at roughly ").append(pct).append("%");
                    first = false;
                }
                sb.append(".");
            }
